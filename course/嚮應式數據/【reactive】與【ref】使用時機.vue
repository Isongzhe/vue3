<template>
  ...
  // 2. reactive定義複雜結構的物件: 重新定義物件時要使用assign
  
  // 使用原則:
  // 1.若需要一個基本類型的響應式數據，使用ref
  // 2.若需要一個響應式物件，且層級不深，使用ref、reactive都可以
  // 3.若需要一個響應式物件，且層級較深，使用reactive(例如:蒐集用戶表單數據大量屬性)
  
  
  
  // Data數據
  let car = ref({brand: 'Toyota', price: 100}) 
  let games = ref([
    {id:'game001', name:'LOL'},
  ...
  //ref 想改變car物件時(改全部，非單一屬性值而已)
  const changeCar = () => {
    car.value = {brand: 'Benz', price: 200} // 可以直接改變
  }
